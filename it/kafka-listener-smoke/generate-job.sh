...
job_name="${K8S_KAFKA_IT_JOB_NAME:-testtender-kafka-listener-it}"
namespace="${K8S_NAMESPACE:-testtender-system}"
image="${K8S_KAFKA_IT_IMAGE:-nicolaka/netshoot:latest}"
docker_host="${K8S_TESTTENDER_DOCKER_HOST:-tcp://testtender:23750}"
container_name="${K8S_KAFKA_IT_CONTAINER_NAME:-kafka-it-smoke}"
        ...
        - name: runner
          ...
          args:
            - |
              ...
              payload="\$(jq -cn '
                {
                  ...
                  Env: [
                    ...
                    "KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://kafka-it-smoke:9092,BROKER://kafka-it-smoke:9093",
              ...
              for i in \$(seq 1 90); do
                if nc -vz -w 2 "\${host}" "\${host_port}" >/dev/null 2>&1; then
                  echo "kafka listener reachable at \${host}:\${host_port}"
                  curl -sS "\${base}/containers/\${cid}/logs?stdout=1&stderr=1&tail=80" | tail -n 80 || true
                  exit 0
                fi
                sleep 1
              done

              echo "kafka listener did not open on \${host}:\${host_port}"
              curl -sS "\${base}/containers/\${cid}/logs?stdout=1&stderr=1&tail=120" | tail -n 120 || true
              exit 1
YAML
