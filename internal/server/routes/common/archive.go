...
func PutArchive(cr *ContextRouter, c *gin.Context) {
	...
	var archiveReader io.Reader = c.Request.Body
	if !tainr.Running && !tainr.Completed && cr.Config.PreArchive {
		...
		if tar.IsSingleFileArchive(archive) {
			...
			// Archive contents may contain credentials or other test fixtures and
			// can be large enough for formatting them to cause a substantial
			// allocation. Log only metadata needed to diagnose pre-start copies.
			klog.V(2).Infof("queued prearchive path=%q bytes=%d total=%d", path, len(archive), len(tainr.PreArchives))
		...
		// Pre-archive init containers support single files. Preserve Docker's
		// multi-entry archive behavior by starting the workload and streaming the
		// archive into it instead.
	...
	// Do not buffer live uploads: Testcontainers legitimately copies files
	// larger than the control-plane memory limit. Peek only far enough to
	// identify compression, then replay those bytes as part of the stream.
	reader := bufio.NewReader(archiveReader)
	...
	if err := cr.Backend.CopyToContainer(tainr, reader, path, tar.IsCompressed(header)); err != nil {
		httputil.Error(c, http.StatusInternalServerError, err)
		return
	}
