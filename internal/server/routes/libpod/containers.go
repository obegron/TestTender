...
import (
	...
	"github.com/obegron/testtender/internal/server/filter"
...
func ContainerCreate(cr *common.ContextRouter, c *gin.Context) {
	...
	requestedImage := in.Image
	resolvedImage, err := cr.ResolveImage(requestedImage)
	if err != nil {
	...
	if img, err := cr.DB.GetImageByNameOrID(requestedImage); err != nil {
...
func ContainerWait(cr *common.ContextRouter, c *gin.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			if tainr.Stopped || tainr.Killed || tainr.Completed {
				c.Data(http.StatusOK, "application/json", []byte(strconv.FormatInt(tainr.ExitCode, 10)))
				return
			}
		}
	}
...
func getContainerInfo(cr *common.ContextRouter, tainr *types.Container, detail bool) gin.H {
	...
	common.UpdateContainerStatus(cr, tainr)
	if detail {
		res["State"] = common.ContainerStateResponse(tainr, errstr)
		res["Config"] = gin.H{
			"Image":  tainr.Image,
			"Labels": tainr.Labels,
			"Env":    tainr.Env,
