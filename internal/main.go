...
import (
	...
	"github.com/obegron/testtender/internal/reaper"
	"github.com/obegron/testtender/internal/server"
)

// Main is the main entry point for starting this service.
func Main() {
	klog.Infof("%s / testtender.io/instance-id=%s", config.VersionString(), config.InstanceID)
...
// getBackend will instantiate the testtender kubernetes object.
...
func run(ctx context.Context, kub backend.Backend) {
	...
	if viper.GetBool("prune-start") {
		klog.Info("pruning all existing testtender resources from namespace")
...
// exitHandler will clean up resources before actually stopping testtender.
func exitHandler(kub backend.Backend, cancel context.CancelFunc) {
	...
	go func() {
		...
		if err := kub.DeleteWithInstanceID(config.InstanceID); err != nil {
			klog.Errorf("error pruning resources: %s", err)
		}
		os.Exit(c)
	}()
