...
// DeleteAll will delete all resources that testtender.io/managed=true
func (in *instance) DeleteAll() error {
	ok := true
	if err := in.deleteServices("testtender.io/managed=true"); err != nil {
		klog.Errorf("error deleting services: %s", err)
		ok = false
	}
	if err := in.deleteConfigMaps("testtender.io/managed=true"); err != nil {
		klog.Errorf("error deleting configmaps: %s", err)
		ok = false
	}
	if err := in.deletePods("testtender.io/managed=true"); err != nil {
...
// DeleteWithInstanceID will delete all resources that have given testtender.io/instance-id
func (in *instance) DeleteWithInstanceID(id string) error {
	ok := true
	if err := in.deleteServices("testtender.io/instance-id=" + id); err != nil {
		klog.Errorf("error deleting services: %s", err)
		ok = false
	}
	if err := in.deleteConfigMaps("testtender.io/instance-id=" + id); err != nil {
		klog.Errorf("error deleting configmaps: %s", err)
		ok = false
	}
	if err := in.deletePods("testtender.io/instance-id=" + id); err != nil {
		klog.Errorf("error deleting pods: %s", err)
		ok = false
	}
	if !ok {
...
func (in *instance) DeleteContainer(tainr *types.Container) error {
	ok := true
	if err := in.deleteServices("testtender.io/container-id=" + tainr.ShortID); err != nil {
		klog.Errorf("error deleting services: %s", err)
		ok = false
	}
	if err := in.deleteConfigMaps("testtender.io/container-id=" + tainr.ShortID); err != nil {
		klog.Errorf("error deleting configmaps: %s", err)
		ok = false
	}
	if err := in.deletePods("testtender.io/container-id=" + tainr.ShortID); err != nil {
		klog.Errorf("error deleting pods: %s", err)
		ok = false
	}
	if !ok {
...
// DeleteOlderThan will delete all testtender created resources older
...
// by testtender and are older than the given keepmax duration.
func (in *instance) DeleteContainersOlderThan(keepmax time.Duration) error {
	pods, err := in.cli.CoreV1().Pods(in.namespace).List(context.Background(), metav1.ListOptions{
		LabelSelector: "testtender.io/managed=true",
	})
	if err != nil {
		return err
	}
	for _, pod := range pods.Items {
		if in.isOlderThan(pod.ObjectMeta, keepmax) {
			...
			if err := in.deleteServices("testtender.io/container-id=" + pod.Name); err != nil {
				klog.Errorf("error deleting services: %s", err)
			}
			if err := in.deleteConfigMaps("testtender.io/container-id=" + pod.Name); err != nil {
				klog.Errorf("error deleting configmaps: %s", err)
			}
...
// DeletePodsOlderThan will delete pods than are orchestrated by testtender
...
func (in *instance) WatchDeleteContainer(tainr *types.Container) (chan struct{}, error) {
	...
	watcher, err := in.cli.CoreV1().Pods(in.namespace).Watch(context.Background(), metav1.ListOptions{
		LabelSelector: "testtender.io/container-id=" + tainr.ShortID,
	})
	if err != nil {
		return nil, err
	}
