...
import (
	...
	"github.com/obegron/testtender/internal/util/tar"
)
...
func (in *instance) startContainer(tainr *types.Container) (DeployState, error) {
	...
	if tainr.Hostname == "" {
		pod.ObjectMeta.Annotations["testtender.io/hostalias-0"] = tainr.GetPodName()
	} else {
		pod.ObjectMeta.Annotations["testtender.io/hostalias-0"] = tainr.Hostname
	}
	for i, hostname := range tainr.NetworkAliases {
		pod.ObjectMeta.Annotations[fmt.Sprintf("testtender.io/hostalias-%d", i+1)] = hostname
	}
	inetwork := 0
	for network := range tainr.Networks {
		pod.ObjectMeta.Annotations[fmt.Sprintf("testtender.io/network-%d", inetwork)] = network
		inetwork++
	}
...
func (in *instance) getServices(tainr *types.Container) []corev1.Service {
	...
	if len(ports) == 0 && len(tainr.NetworkAliases) == 0 && tainr.Hostname == "" {
		return svcs
	}
	...
	for alias := range aliases {
		...
		if len(ports) == 0 {
			// A portless headless Service publishes alias DNS directly to the
			// selected Pod IP. Testcontainers networks do not require clients to
			// declare every port used for container-to-container communication.
...
// as additional labels which are used internally by testtender.
func (in *instance) getLabels(labels map[string]string, tainr *types.Container) map[string]string {
	...
	labels["testtender.io/container-id"] = tainr.ShortID
	return labels
}
...
func (in *instance) getAnnotations(annotations map[string]string, tainr *types.Container) map[string]string {
	...
	annotations["testtender.io/container-name"] = tainr.Name
	return annotations
}
...
func (in *instance) getPodMatchLabels(tainr *types.Container) map[string]string {
	return map[string]string{
		"testtender.io/container-id": tainr.ShortID,
	}
}
...
func (in *instance) GetContainerStatus(tainr *types.Container) (DeployState, error) {
	...
	for _, status := range pod.Status.ContainerStatuses {
		...
		if term != nil {
			updateTerminatedContainerState(tainr, term)
...
// updateTerminatedContainerState copies the workload result reported by the
// kubelet into the Docker-compatible container model. A non-zero workload exit
// is a completed container, not a Kubernetes deployment failure; callers need
// the retained Pod for inspect, wait, logs, and archive operations.
func updateTerminatedContainerState(tainr *types.Container, term *corev1.ContainerStateTerminated) {
	tainr.ExitCode = int64(term.ExitCode)
	tainr.OOMKilled = term.Reason == "OOMKilled"
	tainr.StateError = term.Message
	if !term.StartedAt.IsZero() {
		tainr.Started = term.StartedAt.Time
	}
