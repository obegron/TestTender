...
import (
	...
	"github.com/obegron/testtender/internal"
...
var serverCmd = &cobra.Command{
	Use:   "server",
	Short: "Start the testtender api server",
...
func init() {
	...
	serverCmd.PersistentFlags().Bool("oidc-required", false, "Fail startup unless OIDC authentication is fully configured")
	serverCmd.PersistentFlags().String("oidc-issuer", "", "Exact trusted OIDC issuer from the token iss claim")
	serverCmd.PersistentFlags().String("oidc-discovery-url", "", "OIDC discovery base URL (defaults to the issuer)")
	serverCmd.PersistentFlags().String("oidc-audience", "", "Required OIDC token audience")
	serverCmd.PersistentFlags().String("oidc-allowed-subjects", "", "Comma separated exact OIDC subjects allowed to call the API")
	serverCmd.PersistentFlags().String("oidc-allowed-namespaces", "", "Comma separated exact Kubernetes service-account namespaces allowed to call the API")
	serverCmd.PersistentFlags().String("oidc-ca-file", "", "Additional PEM CA bundle used for OIDC discovery and JWKS")
	serverCmd.PersistentFlags().Duration("oidc-http-timeout", 10*time.Second, "Timeout for OIDC discovery and JWKS requests")
	serverCmd.PersistentFlags().Duration("oidc-jwks-refresh-interval", 15*time.Minute, "Maximum age of the cached OIDC JWKS")
	serverCmd.PersistentFlags().Duration("oidc-clock-skew", 30*time.Second, "Allowed OIDC token clock skew")
	...
	serverCmd.PersistentFlags().String("allowed-secrets", "", "Comma separated exact Secret names workloads may reference")
	serverCmd.PersistentFlags().String("pod-template", "", "Pod file that should be used as the base for creating pods")
	serverCmd.PersistentFlags().String("pod-name-prefix", "testtender", "The prefix of the name to be used in the created pods")
	serverCmd.PersistentFlags().BoolP("inspector", "i", false, "Enable image inspect to fetch container port config from a registry")
	serverCmd.PersistentFlags().String("image-policy-file", "", "JSON policy file used to authorize and rewrite container images")
	...
	serverCmd.PersistentFlags().BoolP("prune-start", "P", false, "Prune all existing testtender resources before starting")
	serverCmd.PersistentFlags().Bool("port-forward", false, "Open port-forwards for all services")
	serverCmd.PersistentFlags().Bool("reverse-proxy", false, "Reverse proxy all services via 0.0.0.0 on the testtender host as well")
	...
	serverCmd.PersistentFlags().Bool("disable-services", false, "Disable service creation (requires a network solution such as testtender-dns)")
	...
	viper.BindPFlag("oidc.required", serverCmd.PersistentFlags().Lookup("oidc-required"))
	viper.BindPFlag("oidc.issuer", serverCmd.PersistentFlags().Lookup("oidc-issuer"))
	viper.BindPFlag("oidc.discovery-url", serverCmd.PersistentFlags().Lookup("oidc-discovery-url"))
	viper.BindPFlag("oidc.audience", serverCmd.PersistentFlags().Lookup("oidc-audience"))
	viper.BindPFlag("oidc.allowed-subjects", serverCmd.PersistentFlags().Lookup("oidc-allowed-subjects"))
	viper.BindPFlag("oidc.allowed-namespaces", serverCmd.PersistentFlags().Lookup("oidc-allowed-namespaces"))
	viper.BindPFlag("oidc.ca-file", serverCmd.PersistentFlags().Lookup("oidc-ca-file"))
	viper.BindPFlag("oidc.http-timeout", serverCmd.PersistentFlags().Lookup("oidc-http-timeout"))
	viper.BindPFlag("oidc.jwks-refresh-interval", serverCmd.PersistentFlags().Lookup("oidc-jwks-refresh-interval"))
	viper.BindPFlag("oidc.clock-skew", serverCmd.PersistentFlags().Lookup("oidc-clock-skew"))
	...
	viper.BindPFlag("kubernetes.allowed-secrets", serverCmd.PersistentFlags().Lookup("allowed-secrets"))
	...
	viper.BindPFlag("kubernetes.request-ephemeral-storage", serverCmd.PersistentFlags().Lookup("request-ephemeral-storage"))
	...
	viper.BindPFlag("registry.policy-file", serverCmd.PersistentFlags().Lookup("image-policy-file"))
	...
	viper.BindEnv("oidc.required", "OIDC_REQUIRED")
	viper.BindEnv("oidc.issuer", "OIDC_ISSUER")
	viper.BindEnv("oidc.discovery-url", "OIDC_DISCOVERY_URL")
	viper.BindEnv("oidc.audience", "OIDC_AUDIENCE")
	viper.BindEnv("oidc.allowed-subjects", "OIDC_ALLOWED_SUBJECTS")
	viper.BindEnv("oidc.allowed-namespaces", "OIDC_ALLOWED_NAMESPACES")
	viper.BindEnv("oidc.ca-file", "OIDC_CA_FILE")
	viper.BindEnv("oidc.http-timeout", "OIDC_HTTP_TIMEOUT")
	viper.BindEnv("oidc.jwks-refresh-interval", "OIDC_JWKS_REFRESH_INTERVAL")
	viper.BindEnv("oidc.clock-skew", "OIDC_CLOCK_SKEW")
	...
	viper.BindEnv("kubernetes.allowed-secrets", "ALLOWED_SECRETS")
	...
	viper.BindEnv("kubernetes.request-ephemeral-storage", "K8S_REQUEST_EPHEMERAL_STORAGE")
	...
	viper.BindEnv("registry.policy-file", "IMAGE_POLICY_FILE")
	viper.BindEnv("kubernetes.timeout", "TIME_OUT")
...
// instantiated by this testtender instance.
