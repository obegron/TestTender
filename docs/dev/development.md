...
TestTender is a minimal Docker API implementation that orchestrates containers on a Kubernetes cluster instead of running them locally. It acts as a drop-in replacement for the Docker API, translating container operations into Kubernetes resources (Pods, Services, ConfigMaps).
...
│   ├── policy/           # TestTender authorization and rewrite policy
...
- Deletes resources owned by terminated testtender instances
...
```bash
# Start testtender with port forwarding
testtender server --port-forward -v 2
...
### Namespace Locking
When multiple testtender instances share a namespace, namespace locking prevents network alias collisions using Kubernetes leases.
